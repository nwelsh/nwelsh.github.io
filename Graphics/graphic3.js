...
function fireworks() {
  ...
  if (genRand(0, 300) < 3) {
    //generate a number between 0 and 300, if its less than 3 so should generate pretty frequently
    //random x, random x and y coordinates to shoot to
    discs.push(
      createDiscs(genRand(0, width), genRand(0, width), genRand(0, height))
    );
  }
  ...
  discs.forEach(function (disc) {
    ...
    if (disc.y <= disc.ty) {
      ...
      while (discs < newDiscs) {
        ...
        boxes.push({
          fade: Math.round(Math.random() * 10) + 20,
          x: disc.tx,
          y: disc.ty,
          r: radius,
          color: disc.c,
